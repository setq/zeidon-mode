(global-set-key (kbd "C-.") (lambda () (interactive) (iterative-search-for-obj)))
(global-set-key (kbd "C-c C-y") (lambda () (interactive) (copy-successful-isearch)))

(defun create-lod-viewer (sObj)
  (interactive "sLod Name: " sObj)
  (setq sObjName sObj)
  (setq workingBuf (current-buffer))
  (setq curPoint (point))
  (setq sGlobalObjBuf "*Obj Buffer*")
  (setq sGlobalEntBuf "*Ent Buffer*")
  ...
  (progn
    (if (get-buffer sGlobalObjBuf)
	(kill-buffer sGlobalObjBuf))
    (generate-new-buffer sGlobalObjBuf)
    (if (get-buffer sGlobalEntBuf)
	(kill-buffer sGlobalEntBuf))
    (generate-new-buffer sGlobalEntBuf))
 (create-lod-view-buffer)
 (switch-to-buffer-other-window workingBuf)
 (goto-char curPoint)
)
...
(defun generate-obj-entity ()
  ...
  (skip-chars-forward " ")
  (setq sObjectName (extract-word))
  (if (not (bufferp sLodView))
       (set-buffer sLodView))
  (insert "Object: " sObjectName)
...
(defun add-global-object ()
  (if (not (bufferp sGlobalObjBuf))
      (set-buffer sGlobalObjBuf))
  (goto-char (point-max))
  (beginning-of-line)
  (insert sObjectName)
  (newline)
...
(defun add-global-entity ()
  (if (not (bufferp sGlobalEntBuf))
      (set-buffer sGlobalEntBuf))
  (goto-char (point-max))
  (beginning-of-line)
  (insert sObjectName "." sEntityName)
  (newline)
)
...
(defun iterative-search-for-obj ()
  (setq curPoint (point))
  (skip-chars-backward "a-z A-Z .")
  (setq obj (extract-word))
  (message obj)
  (switch-to-buffer-other-window sGlobalObjBuf)
  (goto-char 1)
  (if (search-forward obj nil t)
      (progn
	(delete-window)
	(split-window-right 85)
	(switch-to-buffer-other-window sGlobalEntBuf)
	(setq searchBuf sGlobalEntBuf)
	(goto-char 1))
    (progn
...
(defun copy-successful-isearch ()
  ;(set-buffer searchBuf)
  ;(beginning-of-line)
  ;(setq ret (extract-word))
  ;(delete-window)
;  (switch-to-buffer-other-window workingBuf)
;  (delete-window)
  ;(cond ((equal searchBuf sGlobalObjBuf) (insert ret "."))
;	((equal searchBuf sGlobalEntBuf) (insert ret "."))
;	(t nil))
)
